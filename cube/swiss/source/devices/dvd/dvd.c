...
void dvd_set_offset(u64 offset)
{
	if(is_gamecube() && isXenoGC) {
		print_gecko(gettext("XenoGC offset set\r\n"));
		dvd[0] = 0x2e;
		dvd[1] = 0;
		dvd[2] = 0x28000000;
...
void dvd_enable_patches() 
{
	...
	print_gecko(gettext("Drive date %08X\r\nUnlocking DVD\r\n"),driveVersion);
	dvd_unlock();
	print_gecko(gettext("Unlocking DVD - done\r\nWrite patch\r\n"));
	dvd_writemem_array(0xff40d000, patchCode, 0x1F0);
	dvd_writemem_32(0x804c, 0x00d04000);
	print_gecko(gettext("Write patch - done\r\nSet extension %08X\r\n"),dvd_get_error());
	dvd_setextension();
	print_gecko(gettext("Set extension - done\r\nUnlock again %08X\r\n"),dvd_get_error());
	dvd_unlock();
	print_gecko(gettext("Unlock again - done\r\nDebug Motor On %08X\r\n"),dvd_get_error());
	dvd_motor_on_extra();
